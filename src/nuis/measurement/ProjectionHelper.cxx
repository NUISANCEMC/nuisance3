...
ProjectionHelper::Projection(YAML::Node config) {
...
ProjectionHelper::Projection(std::string iname,
...
void ProjectionHelper::Reset() {
    ResetBins();
}

void ProjectionHelper::ResetBins() {
...
int ProjectionHelper::GetBin(const std::vector<double>& invals) {
...
int ProjectionHelper::FillBin(int i, double w) {
    if (i < 0) return -1;
...
int ProjectionHelper::FillBin(const std::vector<double>& indep_vals, double w) {
    int i = GetBin(indep_vals);
    return FillBin(i, w);
}

uint32_t ProjectionHelper::GetMCCounts(const uint32_t i) {
    return mc_counts[i];
}

double ProjectionHelper::GetMCWeight(const uint32_t i) {
    return mc_weights[i];
}

double ProjectionHelper::GetMCError(const uint32_t i) {
    return mc_errors[i];
}

double ProjectionHelper::GetTotalMCCounts() {
    int tot = 0;
...
double ProjectionHelper::GetTotalMCWeight() {
    int tot = 0;
...
double ProjectionHelper::GetTotalMCValue() {
    return GetTotalMCWeight();
}

std::string ProjectionHelper::Summary() {
    return "";
}

void ProjectionHelper::Scale(double s) {
    for (int i = 0; i < mc_counts.size(); i++) {
        mc_counts[i] *= s;
    }
}

void ProjectionHelper::Print() {
    std::cout << Summary() << std::endl;
}

double ProjectionHelper::GetBinContent(int index) {
    return GetMCWeight(index);
}

double ProjectionHelper::GetBinEntries(int index) {
    return GetMCCounts(index);
}

double ProjectionHelper::GetBinError(int index) {
    return GetMCError(index);
}

std::vector<double> ProjectionHelper::GetSlice(
...
std::vector<double> ProjectionHelper::GetXCenter() {
    return GetSlice(bin_center, 0);
}

std::vector<double> ProjectionHelper::GetYCenter() {
    return GetSlice(bin_center, 1);
}

std::vector<double> ProjectionHelper::GetZCenter() {
    return GetSlice(bin_center, 2);
}

std::vector<double> ProjectionHelper::GetXEdge(bool low) {
    return GetSlice(low ? bin_extent_low : bin_extent_high, 0);
}

std::vector<double> ProjectionHelper::GetYEdge(bool low) {
    return GetSlice(low ? bin_extent_low : bin_extent_high, 1);
}

std::vector<double> ProjectionHelper::GetZEdge(bool low) {
    return GetSlice(low ? bin_extent_low : bin_extent_high, 2);
}

std::vector<double> ProjectionHelper::GetXWidth() {
    return GetSlice(bin_width, 0);
}

std::vector<double> ProjectionHelper::GetYWidth() {
    return GetSlice(bin_width, 1);
}

std::vector<double> ProjectionHelper::GetZWidth() {
    return GetSlice(bin_width, 2);
}

std::vector<double> ProjectionHelper::GetMC() {
    return mc_weights;
}
std::vector<double> ProjectionHelper::GetMCErr() {
    return mc_errors;
}

std::vector<double> ProjectionHelper::GetXErr() {
    std::vector<double> v = GetSlice(bin_width, 0);
    for_each(v.begin(), v.end(), [](double &c){ c /= 2.0; });
    return v;
}

std::vector<double> ProjectionHelper::GetYErr() {
    std::vector<double> v = GetSlice(bin_width, 1);
    for_each(v.begin(), v.end(), [](double &c){ c /= 2.0; });
    return v;
}

std::vector<double> ProjectionHelper::GetZErr() {

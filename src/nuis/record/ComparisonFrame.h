...
struct ComparisonFrame {
  ...
  ComparisonFrame(Binning bindef) {
  ...
  template <typename... TS> Eigen::ArrayXd get_content(TS &&...args) {
    return mc.get_content(std::forward<TS>(args)...);
  };

  template <typename... TS> Eigen::ArrayXd get_error(TS &&...args) {
  ...
  template <typename... TS> void reset(TS &&...args) {
    return mc.reset(std::forward<TS>(args)...);
  };

  // Add some data functions
  template <typename... TS> Eigen::ArrayXd get_data_content(TS &&...args) {
    return data.get_content(std::forward<TS>(args)...);
  };

  template <typename... TS> Eigen::ArrayXd get_data_error(TS &&...args) {
  ...
  HistColumn_View operator[](column_t const &colid) const { return mc[colid]; }
};
} // namespace nuis
